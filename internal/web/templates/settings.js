document.addEventListener('DOMContentLoaded', () => {
    ...
    function syncRecurringCategory() {
        const income = document.getElementById('recurringGain').checked;
        const category = document.getElementById('recurringCategory');
        document.getElementById('recurringCategoryField').hidden = income;
    ...
    document.getElementById('saveHousehold').addEventListener('click', async () => { const message = document.getElementById('setupMessage'); try { syncCategories(); const targetInputs = ['essentialsMax', 'lifestyleMax', 'savingsMin'].map(id => document.getElementById(id)); if (targetInputs.some(input => input.value === '')) throw new Error('Set all three monthly guardrails.'); allocationTargets = { essentialsMax: Number(targetInputs[0].value), lifestyleMax: Number(targetInputs[1].value), savingsMin: Number(targetInputs[2].value) }; await E.request('/categories/edit', E.json('PUT', categories)); await Promise.all([E.request('/category-targets/edit', E.json('PUT', categoryTargets)), E.request('/category-parents/edit', E.json('PUT', categoryParents)), E.request('/allocation-targets/edit', E.json('PUT', allocationTargets))]); config.categories = [...categories]; config.categoryTargets = { ...categoryTargets }; config.categoryParents = { ...categoryParents }; config.allocationTargets = { ...allocationTargets }; E.setMessage(message, 'Household guardrails and categories saved.', 'success'); renderCategories(); } catch (error) { E.setMessage(message, error.message, 'error'); } });
    ...
    (async () => { try { [config, recurring] = await Promise.all([E.request('/config'), E.request('/recurring-expenses')]); categories = [...config.categories]; categoryTargets = { ...(config.categoryTargets || {}) }; categoryParents = { ...(config.categoryParents || {}) }; allocationTargets = { essentialsMax: 50, lifestyleMax: 30, savingsMin: 20, ...(config.allocationTargets || {}) }; document.getElementById('essentialsMax').value = allocationTargets.essentialsMax; document.getElementById('lifestyleMax').value = allocationTargets.lifestyleMax; document.getElementById('savingsMin').value = allocationTargets.savingsMin; renderCategories(); renderRecurring(); syncRecurringCategory(); } catch (error) { E.setMessage(document.getElementById('setupMessage'), error.message, 'error'); } })();
});
