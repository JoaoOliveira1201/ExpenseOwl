document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('csvFile').addEventListener('change', async event => { const file = event.target.files[0]; if (!file) return; const message = document.getElementById('importMessage'); const data = new FormData(); data.append('file', file); try { E.setMessage(message, 'Importing…'); const result = await E.request('/import/csv', { method: 'POST', body: data }); E.setMessage(message, `Imported ${result.imported}; skipped ${result.skipped}.`, 'success'); config = await E.request('/config'); categories = [...config.categories]; renderCategories(); } catch (error) { E.setMessage(message, error.message, 'error'); } finally { event.target.value = ''; } });
