document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('saveHousehold').addEventListener('click', async () => { const message = document.getElementById('setupMessage'); try { syncCategories(); await Promise.all([E.request('/categories/edit', E.json('PUT', categories)), E.request('/category-targets/edit', E.json('PUT', categoryTargets))]); config.categories = [...categories]; config.categoryTargets = { ...categoryTargets }; E.setMessage(message, 'Categories and targets saved.', 'success'); renderCategories(); } catch (error) { E.setMessage(message, error.message, 'error'); } });
    ...
    (async () => { try { [config, recurring] = await Promise.all([E.request('/config'), E.request('/recurring-expenses')]); categories = [...config.categories]; categoryTargets = { ...(config.categoryTargets || {}) }; renderCategories(); renderRecurring(); } catch (error) { E.setMessage(document.getElementById('setupMessage'), error.message, 'error'); } })();
});
